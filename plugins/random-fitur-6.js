...
if (command == 'bitly') {
if (!args[0]) throw `Contoh penggunaan ${usedPrefix}${command} https://google.com`
let f = await fetch(`https://api.xteam.xyz/shorturl/bitly?url=${args[0]}&APIKEY=3d628e301bbd3f59`)
let x = await f.json()
let caption = `${x.result}`
await conn.sendButton(m.chat, caption, wm, null, [
                ['Next', `${usedPrefix + command}`]
...
if (command == 'cuttly') {
if (!args[0]) throw `Contoh penggunaan ${usedPrefix}${command} https://google.com`
let f = await fetch(`https://api.xteam.xyz/shorturl/cuttly?url=${args[0]}&APIKEY=3d628e301bbd3f59`)
let x = await f.json()
let caption = `${x.result}`
await conn.sendButton(m.chat, caption, wm, null, [
                ['Next', `${usedPrefix + command}`]
