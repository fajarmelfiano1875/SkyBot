...
let timers = clockString(_ctimer)
  ...
  conn.sendButton(m.chat, caption, clockString(14000), null, [
      ...
      ['Invetory', `${usedPrefix}inv`], ['Profile', `${usedPrefix}profile`]
		...
		setTimeout(() => {
			conn.sendButton(m.chat, _caption, clockString(20000), null, [
      ['Invetory', `${usedPrefix}inv`], ['Profile', `${usedPrefix}profile`]
    ], m, { mentions: conn.parseMention(_caption) })
		}, 18000)

		setTimeout(() => {
			conn.sendButton(m.chat, _caption, clockString(18000), null, [
      ['Invetory', `${usedPrefix}inv`], ['Profile', `${usedPrefix}profile`]
    ], m, { mentions: conn.parseMention(_caption) })
		}, 15000)

		setTimeout(() => {
			conn.sendButton(m.chat, _caption, clockString(15000), null, [
      ['Invetory', `${usedPrefix}inv`], ['Profile', `${usedPrefix}profile`]
    ], m, { mentions: conn.parseMention(_caption) })
		}, 14000)
